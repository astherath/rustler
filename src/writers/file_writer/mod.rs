...
impl OutputBlock {
    ...
    // Calls the md_fmt module to get final formatted Markdown string to write to file
    fn get_markdown_output_str(output_blocks: Vec<OutputBlock>) -> String {
        // TODO:    this function can eventually be re-used if the md_fmt methods are swapped out
        ...
        for block in output_blocks {
            if last_block_type != block.block_type {
                let header_str = md_fmt::get_header_str_for_block_type(&block.block_type);
                output_str.push_str(&header_str);
                last_block_type = block.block_type.clone();
            }

            output_str.push_str(&md_fmt::get_output_str_for_block(block));
        }
        output_str
    }
}
